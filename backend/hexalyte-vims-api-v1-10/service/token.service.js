...
const verifyToken = async (token, type) => {
    try {
        ...
        if (!tokenDoc) {
            throw new ApiError(httpStatus.UNAUTHORIZED, 'Invalid or blacklisted token');
        }

       
        if (moment().isAfter(moment(tokenDoc.expire_at))) {
...
const generateAuthTokens = async (user, rememberMe = false) => {
    ...
    const refreshTokenExpires = rememberMe
        ? moment().add(process.env.JWT_REFRESH_REMEMBER_ME_DAYS, 'days') 
